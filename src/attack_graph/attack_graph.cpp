   ...
   switch (mode) {
      case  TEXT:
        cout << indentation << "RULE " << rulenum << " : " 
             << data0.ruleList.rules[rulenum] << endl;
      break;
...
{
   ostringstream temp;
   temp << "RULE " << rulenum  << " (" << data0.ruleList.rules[rulenum] << ")";
...
int build_graph(void)
{
   ...
   traceStepMap *Map;
   Map = &data0.all_trace_steps.traceSteps;
   for( i=Map->begin(); i != Map->end(); )
   {
      ...
      OrNode *orNode = data0.all_or_nodes.addOrNode(fact_key, f);
      ...
      data0.all_and_nodes.nodeList.add( *andNode );
       graph_data::nodeCount++;
      ...
      for( Fact *fa= c->factList.gethead(); fa > (Fact *) 0; fa = c->factList.getnext()) {
           ...
           if( factType == primitive) {
               newNode = data0.all_leaf_nodes.addLeafNode(fact_key, fa); 
           }
           else if( factType == derived) {
               newNode = data0.all_or_nodes.addOrNode(fact_key, fa); 
           }
   ...
   NodeMap::iterator k;
   for (k = data0.goals.begin(); k != data0.goals.end(); k++) {
     string fact_key = k->first;
     Node *headNode = data0.all_or_nodes.nodes[fact_key];
     if (headNode != NULL){
       data0.goals[fact_key] = headNode;
     }
     else{
   ...
   case noPrune: 
     ...
     /*
   case unFounded: 
     data0.headNode->RemoveUnfoundedEdges();
     break;
     */
   case nonSimple:
     for (k = data0.goals.begin(); k != data0.goals.end(); k++) {
...
//{
//      /*
//      cout << "nodeCount=" << data0.nodeCount << endl;
//      cout << "goal: " << data0.goal->key << endl;

//      int rul =  data0.headNode->rulenum;

//      cout << "headNode: " << data0.headNode->label->key  << endl;

//      cout << "RULES:\n";

//      for ( int i=0; i<= data0.ruleList.count; i++ ) {

//        cout << i << " " ; 

//        if( data0.ruleList.rules[ i ] != 0 ) {  

//          cout <<  data0.ruleList.rules[ i ] ;
...
//      Smap =  &data0.all_trace_steps.traceSteps;
